...
#define H_TILE_WIDTH_X 32
#define H_TILE_WIDTH_y 4
#define H_BLOCK_TILE_COL_STRIDE 8
#define H_BLOCK_TILE_ROW_STRIDE 2
...
__global__ void matMulKernel(float* d_A, float* d_B, float* d_C, int M, int N, int K)
{
    ...
    */
    const int row = blockIdx.y * (BLOCK_TILE_ROW_STRIDE * blockDim.y) + threadIdx.y;
    const int col = blockIdx.x * (BLOCK_TILE_COL_STRIDE * blockDim.x) + threadIdx.x; //Effective BlockSize in Col Dimension. This is also reflected in the grid Dimension declaration.
    ...
    for(int ph = 0; ph<((N+TILE_WIDTH-1)/TILE_WIDTH); ph++)
    {
        ...
        for(int idx_Block1D = 0; idx_Block1D<(BLOCK_TILE_ROW_STRIDE*TILE_WIDTH); idx_Block1D+=TILE_WIDTH)
        {
            if((row + idx_Block1D)<M && (ph*TILE_WIDTH + threadIdx.x)<N)
                MdS[threadIdx.x*BLOCK_TILE_ROW_STRIDE*TILE_WIDTH+ idx_Block1D + threadIdx.y] = d_A[(row + idx_Block1D)*N + (ph*TILE_WIDTH + threadIdx.x)];
            else
                MdS[threadIdx.x*BLOCK_TILE_ROW_STRIDE*TILE_WIDTH+ idx_Block1D + threadIdx.y] = 0.0;
        }
        ...
        for(int i = 0; i<TILE_WIDTH; i++)
        {
            ...
            {
                tempMdS[idx_BlockRow] = MdS[i*BLOCK_TILE_ROW_STRIDE*TILE_WIDTH + idx_BlockRow*TILE_WIDTH+ threadIdx.y]; 
            }
