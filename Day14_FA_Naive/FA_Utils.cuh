...
__device__ void __inline__ load_K_V(float* K, float* V, float* shK_T, float* shV, int N, int d)
{
...
__device__ void __inline__ load_Q(float* Q, float* shQ, int N, int d)
{
    const int idxRow = threadIdx.x/d;
    const int idxCol = threadIdx.x%d;
    const int stride = blockDim.x/d;
...
template<const int BM=32>
__device__ void __inline__ load_O(float* O, float* shO, int d)
{
    assert(blockDim.x>=d);
    const int idxRow = threadIdx.x/d;
    const int idxCol = threadIdx.x%d;
...
template<const int BM=32>
__device__ void __inline__ load_L_i_M_i(float* shM_i, float* shL_i, float* m, float* l)
{
...
__device__ __inline__ void rowMax_calculateP_rowSum(float* shS_P, float* shM_ij, float* shL_ij)
{
    assert(BN <= WARPSIZE);
...
//Step 11
template<const int BM=32>
__device__ __inline__ void calculate_Mnew_i_Lnew_i(float* shM_i, float* shL_i, float* shM_ij, float* shM_New_i, float* shL_New_i, float* shL_ij)
{
    assert(BM <= blockDim.x);
    if(threadIdx.x < BM)
    {
...
template<const int BM=32>
__device__ __inline__ void copy_L_i_M_i(float* shM_i, float* shL_i, float* shM_New_i, float* shL_New_i)
{
    assert(BM <= blockDim.x);
    if(threadIdx.x < BM)
    {
...
template<const int BM=32>
__device__ __inline__ void write_O(float* O, float* shO, int d)
{
    const int idxRow = threadIdx.x/d;
    const int idxCol = threadIdx.x%d;
    const int stride = blockDim.x/d;
