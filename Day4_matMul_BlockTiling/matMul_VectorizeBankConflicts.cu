...
__global__ void matMulKernel(float* d_A, float* d_B, float* d_C, int M, int N, int K)
{
    ...
    */
    ...
    for(int ph = 0; ph<((N+TILE_WIDTH-1)/TILE_WIDTH); ph++)
    {
        ...
        for(int idx_Block1D = 0; idx_Block1D<BLOCK_TILE_ROW_STRIDE; idx_Block1D++)
        {
            if((row + idx_Block1D*TILE_WIDTH)<M && (ph*TILE_WIDTH + 4*threadIdx.x)<N && 4*threadIdx.x < TILE_WIDTH) //Only 1/4th the threads in a block are launched
            {
                reinterpret_cast<float4*>(&MdS[(threadIdx.y + idx_Block1D*TILE_WIDTH)*TILE_WIDTH + 4*threadIdx.x + 0])[0] = reinterpret_cast<float4*>(&d_A[(row + idx_Block1D*TILE_WIDTH)*N + (ph*TILE_WIDTH + 4*threadIdx.x)])[0];
            }
        }
        
        for(int idx_Block1D = 0; idx_Block1D<BLOCK_TILE_COL_STRIDE; idx_Block1D++)
        {
            if((ph*TILE_WIDTH + threadIdx.y)<N && (col + idx_Block1D*TILE_WIDTH - threadIdx.x + 4*threadIdx.x)<K)
            {
