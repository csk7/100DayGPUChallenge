...
__global__ void __launch_bounds__((BM*BN)/(TM*TN),1) matMulKernel(float* d_A, float* d_B, float* d_C, int M, int K, int N)
{
    ...
    const  uint bx = blockIdx.y*(N/BN) + blockIdx.x; //Linearize BlockIndex
    const uint swizzleBlockIdx = bx / (SWIZZLE*SWIZZLE); //Each block is 4*4, so dividing by it (Eq to WARPSIZE)
    const uint bR = swizzleBlockIdx / ((N / BN)/SWIZZLE); //It varies between no.of blocks in X dir and SWIZZLE(4)
    const uint bC = swizzleBlockIdx % ((N / BN)/SWIZZLE);

    const uint swizzleSubBlockIdx = bx % (SWIZZLE*SWIZZLE); //%to get to Sub Block
    const uint bSR = swizzleSubBlockIdx / SWIZZLE; //It varies between Swizzle and 1. There we had TN even smaller unit.
    const uint bSC = swizzleSubBlockIdx % SWIZZLE;
    // index inside the swizzle block
    const uint cCol =
        bC * SWIZZLE + bSC; //Only cols idx above affect col.
    const uint cRow =
        bR * SWIZZLE + bSR; //Only rows idx above affect row.
    ...
    if(threadIdx.x == 0)//if(block.thread_rank() == 0)
    {
        init(&frontBarrier, blockDim.x);//block.size());
        init(&backBarrier, blockDim.x);//block.size());
    }
    __syncthreads();
    ...
    d_A += cRow*BM*K; //Reading the Global input array is same
    d_B += cCol*BN; 
    d_C += ((cRow*BM + warpRow*WM)*N  + (cCol*BN + warpCol*WN)); //Output, each thread write a sub warp tile. Here we go the warp inside block.
