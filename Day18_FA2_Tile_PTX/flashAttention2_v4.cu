...
__global__ void flashAttention2(const nv_bfloat16* Q, const nv_bfloat16* K, const nv_bfloat16* V, nv_bfloat16* O, const int seqLength, int batchSize)
{
    ...
    {
        const int idxRowV = laneId%32;
        const int idxColV = laneId/32*8;
    ...
    for(int idx_KV = 0; idx_KV<(seqLength/Bc); idx_KV++)
    {
        ...
        {
            for(int mma_id_col=0; mma_id_col<d; mma_id_col+=(2*MMA_K))
            {
                ...
                sharedToRegx4(Kreg[mma_id_row/MMA_N][mma_id_col/(2*MMA_K)], srcShAddress);
            }
        }
        //MMA
        for(int i=0; i<(blockQperWarp/MMA_M); i++)
        ...
        {
            for(int mma_j_col=0; mma_j_col<(d/MMA_N); mma_j_col+=2)
            {
                ...
                sharedToRegx4Trans(Vreg[mma_i_row][mma_j_col], srcShAddress);
            }
        }
...
void flashAttention2_v4(const nv_bfloat16* Q, const nv_bfloat16* K, const nv_bfloat16* V, nv_bfloat16* O, int seqLength, int batchSize)
{    
    const int Br = 64;
    const int Bc = 32;
    const int d = 128;
